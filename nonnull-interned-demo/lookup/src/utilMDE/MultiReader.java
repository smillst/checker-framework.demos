...
public class MultiReader implements Iterable<String>, @NonNull Iterator<@NonNull String> {
  ...
  public @Nullable String readLine() throws IOException {
    ...
    @Nullable String line = get_next_line();
    if (comment_re != null) {
      while (line != null) {
    ...
    if (include_re != null) {
      ...
      if (m.matches()) {
        @Nullable String fname_group = m.group(1);
        if (fname_group == null) {
          throw new IOException
            (String.format ("Include file regular expression '%s' matched "
                            + "line '%s' but group one (which should match the "
                            + "filename) did not match anything",
                            include_re, line));
          }
        File filename = new File (UtilMDE.fix_filename(fname_group));
        // System.out.printf ("Trying to include filename %s%n", filename);
        if (!filename.isAbsolute()) {
          File current_filename = new File (readers.peek().filename);
          @Nullable File current_parent = current_filename.getParentFile();
  ...
  public String next() {
    try {
        @Nullable String tmp = readLine();
        if (tmp == null)
            throw new Error("no more to read");
        return tmp;
    } catch (IOException e) {
      throw new Error ("unexpected IOException", e);
    }
  ...
  public @Nullable Entry get_entry() throws IOException {
    ...
    @Nullable Matcher entry_match = null;
    if (entry_start_re != null)
      entry_match = entry_start_re.matcher (line);
    if ((entry_match != null) && entry_match.find()) {

      // At this point we know that start_re was specified (and found)
      // We presume that a stop_re is always present when a start one is.
      assert entry_start_re != null; // @NE app-inv
      assert entry_stop_re != null;  // @NE app-inv

      // Remove entry match from the line
      if (entry_match.groupCount() > 0) {
        line = entry_match.replaceFirst ((@NonNull String)entry_match.group(1)); // @NE complex-check
      }
  ...
  private @Nullable String get_next_line() throws IOException {
    ...
    @Nullable String line = ri.reader.readLine();
    ri.line_number++;
    while (line == null) {
      readers.pop();
      if (readers.empty())
